...
const Contact = ({ defaultMessage }) => {
  ...
  const refreshCaptchaToken = async () => {
    ...
    const newToken = await executeRecaptcha("contactFormSubmit");
    setToken(newToken);
  ...
  const handleSubmit = async (e) => {
    ...
    await POST(`/contact/contact_page?form_section=${contactPage}`, {
      ...
      // <<< OSVEŽI TOKEN NAKON SVAKOG POKUŠAJA >>>
      await refreshCaptchaToken();
    });
  };
            ...
            <div className="flex w-[300px] flex-col gap-4">
              {textOptions.map((option) => (
                <button
                  key={option.id}
                  onClick={() =>
                    handleSetPredefinedText(option.messageText, option.buttonText)
                  }
                  type="button"
