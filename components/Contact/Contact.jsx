...
const ContactForm = ({ defaultMessage }) => {
  ...
  const handleChange = ({ target }) => {
    const updatedErrors = errors.filter((error) => error !== target.name);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!executeRecaptcha) {
      toast.error("reCAPTCHA nije spremna. Pokušajte ponovo.");
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (res?.code === 200) {
        ...
        // Resetovanje tokena posle uspešnog slanja
        if (executeRecaptcha) {
          ...
          {contactPageOptions.map((option) => (
            ...
            >
              <SvgButtonOne
                ...
                fill={
                  contactPage === option.value ? "#224e35" : "transparent"
                }
              />
              <div
